...
const CommonBannerThree = ({ title, main_title, description, brandName }: {title: string, main_title: string, description: string, brandName?: string }) => {
  return (
    <div style={{
      ...
      borderRadius: "20px"
    }} className="lg:m-8 m-4 rounded-3xl md:pt-24 py-20 lg:pb-56 xl:pb-76 2xl:pb-[520px] px-4">
      <div className="mb-8">
        <CommonSubTitle title={title || ""} />
      </div>
      <h2 className="text-[#151515] text-3xl sm:text-[36px] md:text-[48px] lg:text-[56px] xl:text-[64px] font-semibold text-center max-w-[1080px] mx-auto">{main_title}</h2>
